...
#from utils import chat,get_gemini_response
...
def Kisan_mitra(lang,prompt):
    ...
    if prompt:
        with st.spinner(text='Wait...I am answering...'):
            if lang=="English":
             response = chat(prompt=prompt,system_prompt=input_prompt_e,)
             if response:
              st.success('Done')
              st.write(response)
            elif lang=="Hindi":
             response = chat(prompt=prompt,system_prompt=input_prompt_h,)
             if response:
              st.success('Done')
              st.write(response)
